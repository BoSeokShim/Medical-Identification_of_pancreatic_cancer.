...
def get_df_stone(k_fold, data_dir, data_folder, out_dim = 1, use_meta = False, use_ext = False):
    ...
    ####################################################
    '''
    diagnosis2idx = {d: idx for idx, d in enumerate(sorted(df_train.patient.unique()))} #문자열을 index값으로 매핑
    ...
    return df_train, 0, meta_features, n_meta_features, target_idx
...
class MMC_ClassificationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_meta:
            data = (torch.tensor(image).float(), torch.tensor(self.csv.iloc[index][["Mean RI","Concentration (pg/um³)"]]).float())
            # "Volume (um³)","Surface area (um²)","Concentration (pg/um³)","Dry mass (pg)","Lipid Volume (um^3)","Lipid Mass (pg)","Projected area (um²)","Mean RI","Sphericity","Threshold RI"
        else:
            data = torch.tensor(image).float()

        if self.mode == 'valid':
            # Test 의 경우 정답을 모르기에 데이터만 리턴
            return data, torch.tensor(self.csv.iloc[index].target), patient_id, self.csv.iloc[index].image_name,self.csv.iloc[index].cancer  # , patient_id
        else:
            # training 의 경우 CSV의 스톤여부를 타겟으로 보내줌
            return data, torch.tensor(self.csv.iloc[index].target),self.csv.iloc[
                index].image_name
