...
def get_df_stone(k_fold, data_dir, data_folder, out_dim = 1,out_dim2 = 1, use_meta = False, use_ext = False):
    '''
    ...
    # meta_columns 들을 다 불러옴
    meta_columns = list(df_train.columns)[3:]
    for i in ['patient', 'cancer']:
        meta_columns.remove(i)
    ...
    people_ids = df_train['Title'].unique()
    num_people = len(people_ids)

    print(f'Original dataset의 수 : {num_people}')
    ...
    df_train['fold'] = df_train['Title'].map(faceid2fold)
    ...
    if use_ext:
        ...
        patients = len(df_train_ext['Title'].unique())
    ...
    if use_meta:
        meta_features = True
        n_meta_features = len(meta_columns)
        # meta_features = 1
        # n_meta_features = 1
    else:
        meta_features = None
    ...
    ####################################################
    '''
    diagnosis2idx = {d: idx for idx, d in enumerate(sorted(df_train.patient.unique()))}
    df_train['target'] = df_train['patient'].map(diagnosis2idx)

    diagnosis2idx2 = {d: idx for idx, d in enumerate(sorted(df_train.cancer.unique()))}
    df_train['target2'] = df_train['cancer'].map(diagnosis2idx2)

    # # test data의 레이블 정보 (없는 경우엔 필요없음)
    # if 'target' in df_test.columns:
    #     df_test['patient'] = df_test['patient'].map(diagnosis2idx)
    ...
    return df_train, df_test, meta_features, n_meta_features, target_idx, meta_columns
...
class MMC_ClassificationDataset(Dataset):
    ...
    def __init__(self, csv, mode, meta_features, transform=None, meta_columns=None):
    ...
    def __getitem__(self, index):
        ...
        patient_id = row['Title']
        # 메타 데이터를 쓰는 경우엔 image와 함께 텐서 생성
        if self.use_meta:
            # data = (torch.tensor(image).float(), torch.tensor(self.csv.iloc[index][['Cell Count', 'Concentration (pg/um³)']]).float())
            data = (torch.tensor(image).float(),
                    torch.tensor(self.csv.iloc[index][self.meta_columns]).float())
        else:
            data = torch.tensor(image).float()

        if self.mode == 'valid':
            # Test 의 경우 정답을 모르기에 데이터만 리턴
            return data, torch.tensor(self.csv.iloc[index].target), patient_id, self.csv.iloc[
                index].image_name, self.csv.iloc[index].cancer  # , patient_id
        else:
            # training 의 경우 CSV의 스톤여부를 타겟으로 보내줌
            return data, torch.tensor(self.csv.iloc[index].target), torch.tensor(self.csv.iloc[index].target2), self.csv.iloc[index].image_name #, torch.tensor(self.csv.iloc[index].target2)
            # return {"train_data ": image , "label": 12 }

        #return data, torch.tensor(self.csv.iloc[index].patient).long()
